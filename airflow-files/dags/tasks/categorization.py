...
def CategorizationOperator(migration_order, files_path, schemas_path, out_path, log_path, aws_connector, aws_bucket, aws_path, db_connector, tablename):
    log_file = log_path + 'categorization.log'
    f = open(log_file, 'w')
    f.write('{}\n\n'.format(datetime.now()))
    ...
    try:
        ...
        if order_dict['categorization_status'] == True: 
            f.write('Already did')
            return
    except:
    ...
    try:
        a = RunSQL(
            ...
            sql = f'''
                UPDATE {tablename}
                SET categorization_status = false,
                    categorization_date = '{datetime.today().strftime('%Y-%m-%d')}'
    ...
    scripts_path = schemas_path + '/' + system + '/' + version + '/categorization_scripts/'
    for s in os.listdir(scripts_path):
        ...
        if os.path.isfile(csv_file):
            ...
            model_columns = {}
            for key, value in cat_script_data['Categorization'].items():
    ...
    try:
        a = RunSQL(
            ...
            sql = f'''
                UPDATE {tablename}
                SET categorization_status = true
                WHERE order_id = {order_id}
                '''
        )
    except:
